  ...
  methods: {
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/ecmall4cloudproduct/attr/sale/list/${this.spu.catalogId}`
          ),
          method: 'get',
          params: this.$http.adornParams({
            page: 1,
    ...
    showBaseAttrs () {
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(
            `/ecmall4cloudproduct/attrgroup/${this.spu.catalogId}/withattr`
          ),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {
    ...
    submitSkus () {
      ...
      this.$confirm('将要提交商品数据，需要一小段时间，是否继续?', '提示', {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl('/ecmall4cloudproduct/spuinfo/save'),
            method: 'post',
    ...
    descartes (list) {
      ...
      // 动态生成笛卡尔积
      while (true) {
        for (var indexx in list) {
          tempCount = point[indexx]['count']
          temp.push(list[indexx][tempCount])
        }
