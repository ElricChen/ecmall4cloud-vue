      ...
      if (path && path.length === 3) {
        this.$http({
          url: this.$http.adornUrl(
            `/ecmall4cloudproduct/attrgroup/list/${path[path.length - 1]}`
          ),
          method: 'get',
    ...
    init (id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.attrId) {
          this.$http({
            url: this.$http.adornUrl(
              `/ecmall4cloudproduct/attr/info/${this.dataForm.attrId}`
            ),
            method: 'get',
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/ecmall4cloudproduct/attr/${!this.dataForm.attrId ? 'save' : 'update'}`
            ),
            method: 'post',
